...
<div class="form-group row">
    ...
    <option value="{{$user->id}}" {{ $user->id == $repairOrders->userId ? 'selected' : '' }}>{{$user->name}}</option>
    @endforeach
    </select>
...
<input class="form-check-input" name="repair_type[]" type="checkbox" value="{{$repair->repair_type}}" id="{{$repair->id}}" {{  ($repairOrders->id == $repair->order_Id ? ' checked' : '') }} >
...
<div class="col-md-10">
<input class="form-control" name="date"  type="date" placeholder="Select date" title="Select Date"  value="{{$repairOrders->date}}"  id="txtDate">
...
<div id="brandname"><h5>Brand Name:<b style="float: center"> {{$model->brand->brand_name}}</b><h5></div>
